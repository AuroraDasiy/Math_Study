%sum:计算向量测度
%advance:测度计算可用AHP方法/熵权法计算指标的权重再算测度
  %熵权法:
   %各个样本的概率pi,-ln(pi)为信息量 ,信息量的期望值就是熵权
    %各个样本概率相同期望最大为-ln(n) 归一化 1-E/-ln(n)  熵权越大信息量越小所以1-之  归一
...
%% 第四步：计算与最大值的距离和最小值的距离,用层次分析计算各指标权重来算新的测度，并算出得分
